...
export default function AppointmentCard({
  ...
  const handleAccept = async () => {
    ...
    try {
      const token =
        (await AsyncStorage.getItem("BusinessToken")) ||
      ...
      await axios.patch(
        `https://${BASE_URL}/api/appointments/${appointment.id}/status`,
        { status: "booked" },
        { headers }
      );
      if (typeof arguments[0].onAccept === "function")
        arguments[0].onAccept({ ...appointment, status: "booked" });
    } catch (e) {
  ...
  const handleDecline = async () => {
    ...
    try {
      ...
      if (typeof arguments[0].onDecline === "function")
        arguments[0].onDecline({ ...appointment, status: "canceled" });
    } catch (e) {
      if (typeof e === "object" && e !== null && "response" in e) {
        // @ts-ignore
        ...
        <View style={styles.content}>
          ...
          {appointment.staffName && (
            <View style={styles.infoRow}>
              ...
              >{`Staff: ${appointment.staffName}`}</Text>
            </View>
          )}
          ...
          <View style={styles.infoRow}>
            <Text
              style={[
                styles.infoText,
                { color: Colors.status.success, fontWeight: "600" as const },
              ]}
            >
              {appointment.servicePrice} UZS
            </Text>
          </View>
